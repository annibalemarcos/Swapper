...
def get_crypto_rates():
    """Simula busca de taxas do sistema swap"""
    # Em produção, buscar de /api/pix/rates
    return {
        'D1': 15.50,
        'D2': 22.30,
        'D3': 8.75,
    ...
    while True:
        try:
            ...
            for invoice in pending:
                ...
                # CORREÇÃO: Arredondar valores antes de comparar
                balance_rounded = round(balance, 2)
                payment_amount_rounded = round(invoice['payment_amount'], 2)
...
def check_deposit_balance(address, crypto):
    ...
    # Em produção, fazer request para swap API
    # Aqui retorna random para simular
    if random.random() > 0.95:  # 5% de chance de "receber" pagamento
...
def api_select_crypto():
    ...
    # ARREDONDAR PARA 2 CASAS DECIMAIS (compatível com validação)
    crypto_amount = round(invoice['amount_brl'] / rates.get(crypto, 10), 2)
