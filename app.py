...
def monitor_swap_deposits():
    """Monitora depósitos em endereços de swap e vendas PIX"""
    global swap_monitor_active
    
    log_info("🔍 Monitor de depósitos iniciado (Crypto + PIX)")
    ...
    while swap_monitor_active:
        try:
            ...
            # Processar vendas PIX (Crypto → PIX)
            cursor.execute("""
                SELECT * FROM swap_orders 
                WHERE status = 'pending' AND order_type = 'pix_sell'
            """)
            pix_sell_orders = cursor.fetchall()
            
            for order in pix_sell_orders:
                deposit_balance = get_wallet_balance(order['from_coin_id'], order['deposit_address'])
                
                if deposit_balance >= order['from_amount']:
                    # DEPÓSITO DETECTADO! Processar venda PIX
                    cursor.execute('SELECT symbol FROM coins WHERE id = ?', (order['from_coin_id'],))
                    coin_symbol = cursor.fetchone()['symbol']
                    
                    # 1. Descontar taxa
                    fee_amount = order['fee_amount']
                    fee_wallet = f"FEE_WALLET_{coin_symbol}"
                    transfer_transaction(order['from_coin_id'], order['deposit_address'], fee_wallet, fee_amount)
                    log_info(f"💸 Taxa de {fee_amount:.4f} {coin_symbol} ({order['fee_percentage']:.2f}%) coletada")
                    
                    # 2. Queimar o restante das moedas
                    remaining_amount = order['from_amount'] - fee_amount
                    burn_transaction(order['from_coin_id'], order['deposit_address'], remaining_amount)
                    log_info(f"🔥 {remaining_amount:.4f} {coin_symbol} queimados")
                    ...
                    log_success(f"💰 VENDA PIX COMPLETA!")
                    log_success(f"   Recebido: {order['from_amount']:.4f} {coin_symbol}")
                    log_success(f"   PIX enviado: R$ {order['to_amount']:.2f} para {order['pix_key']}")
                    log_pix('sell', order['to_amount'], coin_symbol)
    ...
    log_warning("🛑 Monitor de swaps finalizado")
...
def admin_panel():
    ...
    cursor.execute("""
        SELECT t.*, c.symbol, b.block_number 
        FROM transactions t
    ...
    cursor.execute("""
        SELECT s.*, c1.symbol as from_symbol, c2.symbol as to_symbol
        FROM swaps s
    ...
    cursor.execute("""
        SELECT so.*, c1.symbol as from_symbol, c2.symbol as to_symbol
        FROM swap_orders so
    ...
    cursor.execute("""
        SELECT so.*, c1.symbol as coin_symbol
        FROM swap_orders so
...
@app.route('/api/wallet/<int:coin_id>/<address>')
def api_get_wallet_balance(coin_id, address):
    """Retorna saldo de uma carteira específica - USADO PELO GATEWAY"""
    balance = get_wallet_balance(coin_id, address)
